/**
 * Dashboard Manager
 * Orchestrates Data Loading, Filtering, and Component Updates.
 */
...
// Trend Chart State
// Modes: "state_severity", "state_count", "weather_severity", "weather_count"
...
// Helper to map state to Census Region
function getCensusRegion(state) {
    if (!state) return "Unknown";
    for (const [region, set] of Object.entries(US_CENSUS_REGIONS)){ 
...
d3.csv("./US_Accidents_March23_sampled_500k.csv", d => {
    ...
    const temp = d["Temperature(F)"] || d["Temperature(F) "];
    const hum = d["Humidity(%)"] || d["Humidity(%) "];
...
function setupControls() {
    ...
    if(trendModeSelect){
        ...
        trendModeSelect.addEventListener("change", function() {
            trendMode = this.value; 
            updateDashboard();
        });
    }
    ...
    weatherSelect.addEventListener("change", function() {
        if (this.value) { 
            selectedWeather.add(this.value); 
            renderWeatherTags(); 
            updateDashboard(); 
            this.value = ""; 
        }
    ...
    if(regionSelect) regionSelect.addEventListener("change", function() {
        ...
        if (currentRegionMode !== "Manual" && REGIONS[currentRegionMode]) {
    ...
    if (dateExtent[0]) {
        dateStartInput.min = formatDateInput(dateExtent[0]);
        dateStartInput.max = formatDateInput(dateExtent[1]);
        dateEndInput.min = formatDateInput(dateExtent[0]);
        dateEndInput.max = formatDateInput(dateExtent[1]);
    }
    // Default to 2021
    ...
    function handleDateChange() {
        ...
        if(!isNaN(s) && !isNaN(e)) {
            e.setHours(23, 59, 59);
            currentDateRange.start = s; currentDateRange.end = e;
            updateDashboard();
        }
    }
...
function refreshWeatherDropdown(dataContext) {
    ...
    const weatherCounts = d3.rollups(dataContext, v => v.length, d => d.weather)
        .sort((a, b) => b[1] - a[1]); 
...
function updateDashboard() {
    // A. Time Filtering (Global Scope)
    const dateFilteredData = globalData.filter(d => 
        d.date >= currentDateRange.start && d.date <= currentDateRange.end
    );
    const globalTotal = dateFilteredData.length;

    // B. Map Data: Date + Weather (Ignores State Selection)
    const mapData = dateFilteredData.filter(d => 
        selectedWeather.size === 0 || selectedWeather.has(d.weather)
    );
    const mapTotalAccidents = mapData.length;

    // C. Chart/Dropdown Data: Date + Weather + State
    const stateFilteredData = mapData.filter(d => 
        selectedStates.size === 0 || selectedStates.has(d.state)
    );

    // Context for Weather Dropdown
    const dropdownContext = dateFilteredData.filter(d => 
        selectedStates.size === 0 || selectedStates.has(d.state)
    );
    ...
    // 4. Trend Chart (Dynamic Title + Data)
    ...
    let titleMetric = (metricType === "count") ? "Accident Count" : "Avg Severity";
    let titleGroup = (groupType === "state") ? "State" : "Weather";

    if (groupType === "state") {
        trendData = dateFilteredData.filter(d => (selectedWeather.size === 0) || selectedWeather.has(d.weather));
        
        if (selectedStates.size === 0) {
            groupByFunc = (d) => "National (All States)";
            filterSet = null; 
        } else {
            groupByFunc = (d) => d.State;
            filterSet = selectedStates; 
        }
    } 
    ...
    if (h2) {
        h2.textContent = `Daily ${titleMetric} by ${titleGroup}`;
    }

    if (regionalTrendChart) {
        regionalTrendChart.setData(trendData, {
            metric: metricKey,
            groupBy: groupByFunc,
            filterSet: filterSet
        });
...
function updateSelectionLabel(stateStats, globalTotal) {
    ...
    if (selectedStates.size > 0) {
        selectedStates.forEach(abbr => {
            ...
            if(s) currentTotal += s.accidents;
        });
    } else {
        stateStats.forEach(s => currentTotal += s.accidents);
    }
    ...
    if (selectedStates.size === 0) {
        box.innerHTML = `
            ...
            <div class="muted">All States Included</div>
            <div style="border-top:1px solid #eee; padding-top:5px; margin-top:5px;">
                <strong>${d3.format(",")(currentTotal)}</strong> Accidents <span class="muted">(${globalShare.toFixed(1)}%)</span>
            </div>
        `;
        return;
    }
    ...
    selectedStates.forEach(abbr => {
        const s = stateStats.get(abbr);
        if (s && s.accidents > 0) {
            const share = (s.accidents / currentTotal) * 100;
            breakdown.push({ name: abbr, pct: share });
        }
    });
    ...
    box.innerHTML = `
        ...
        <div style="font-size:0.7rem; line-height:1.3; color:#555; margin-bottom: 8px;">
            ${listStr}
        </div>
        <div style="border-top:1px solid #eee; padding-top:5px;">
            <strong>${d3.format(",")(currentTotal)}</strong> Accidents <span class="muted">(${globalShare.toFixed(1)}% of Country)</span>
        </div>
    `;
}
...
function updateWeatherLabel(finalData, totalContextCount) {
    ...
    if (selectedWeather.size === 0) {
        box.innerHTML = `
            <div><strong>All Weather Conditions</strong></div>
            <div class="muted">No specific weather filtered</div>
            <div style="border-top:1px solid #eee; padding-top:5px; margin-top:5px;">
                <strong>${d3.format(",")(currentTotal)}</strong> Accidents <span class="muted">(${globalShare.toFixed(1)}%)</span>
            </div>
        `;
    ...
    const breakdown = counts.map(([name, count]) => ({
        name: name,
        pct: (count / currentTotal) * 100
    ...
    box.innerHTML = `
        ...
        <div class="muted" style="margin-bottom:4px;">${selectedWeather.size} Condition${selectedWeather.size > 1 ? 's' : ''} Selected</div>
        <div style="font-size:0.7rem; line-height:1.3; color:#555; margin-bottom: 8px;">
            ${listStr}
        </div>
        <div style="border-top:1px solid #eee; padding-top:5px;">
            <strong>${d3.format(",")(currentTotal)}</strong> Accidents <span class="muted">(${globalShare.toFixed(1)}% of Selection)</span>
        </div>
    `;
}
...
function updateBarChart(data, groupType, metricType) {
    ...
    if (groupType === "state") {
        ...
        groupFunc = d => d.weather;
        label = "Top Weather Conditions";
    }

    const rollup = d3.rollups(data, v => {
        const count1 = v.filter(d => d.severity === 1).length;
        const count2 = v.filter(d => d.severity === 2).length;
        const count3 = v.filter(d => d.severity === 3).length;
        const count4 = v.filter(d => d.severity === 4).length;
        const total = v.length;
        const avgSev = total > 0 ? (1*count1 + 2*count2 + 3*count3 + 4*count4) / total : 0;
    ...
    if (metricType === "count") {
        ...
        if (h2) h2.textContent = `${label} by Accident Count`;
    } else {
        barData.sort((a, b) => b.avgSev - a.avgSev);
        if (h2) h2.textContent = `${label} by Average Severity`;
    }
    ...
    if(typeof drawStackedBarChart === "function") {
        drawStackedBarChart(svg, barData, color);
    }
}